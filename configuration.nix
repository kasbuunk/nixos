...

{
  ...
  services = {
    ...
    crowdsec = {
      ...
      settings.general = {
        ...
        prometheus = {
          ...
          listen_port = cfg.services.crowdsec.metricsPort;
        };
      };
      ...
      localConfig.acquisitions = [
        {
          ...
          journalctl_filter = [
            ...
            "_SYSTEMD_UNIT=wireguard-wgserver.service"
          ];
          labels.type = "syslog";
        }
      ];
    ...
    # The Firewall Bouncer (Blocks the IPs detected by the engine).
    crowdsec-firewall-bouncer = {
      enable = true;

      # It defaults to blocking via nftables/iptables.
      # It automatically looks for the local API on 127.0.0.1:8080
    };
    ...
    mimir = {
      ...
      configuration = {
        ...
        common = {
          storage = {
            backend = "filesystem";
            filesystem.dir = cfg.services.mimir.dataDir;
          };
        };

        blocks_storage = {
          backend = "filesystem";
          bucket_store.sync_interval = "1m";
          filesystem.dir = "${cfg.services.mimir.dataDir}/blocks";
          tsdb = {
            dir = "${cfg.services.mimir.dataDir}/tsdb";
          };
        };
